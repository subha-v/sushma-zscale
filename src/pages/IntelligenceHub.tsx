...
const FOUNDER_MANUAL_DOWNLOAD_URL = 'https://drive.google.com/uc?export=download&id=YOUR_FOUNDER_MANUAL_FILE_ID'; // Update with actual Founder Manual PDF ID
...
export const IntelligenceHub = () => {
  ...
  const handlePlaybookClick = (playbookId: string) => {
    ...
    // Free downloads - no email required for PMF Audit, Shadow Capital, Founder Manual
    if (playbookId === 'pmf-audit' || playbookId === 'shadow-capital' || playbookId === 'founder-manual') {
      // Download immediately without email capture
      if (playbookId === 'pmf-audit') {
        triggerDownload();
        showSuccessToast();
      } else if (playbookId === 'shadow-capital') {
        triggerShadowCapitalDownload();
        setToastMessage('Intelligence Unlocked. Your 2026 Shadow Capital Report is downloading now.');
        setShowToast(true);
      } else if (playbookId === 'founder-manual') {
        // Trigger Founder Manual download
        const link = document.createElement('a');
        link.href = FOUNDER_MANUAL_DOWNLOAD_URL;
        link.setAttribute('download', 'zScale-Founder-Manual-2026.pdf');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        setToastMessage('Intelligence Unlocked. Your Founder Manual is downloading now.');
        setShowToast(true);
      }
                  ...
                  >
                    ...
                    <div className={isLocked ? 'blur-sm' : ''}>
                      ...
                      {isAlphaMember ? (
                        ...
                        <Link
                          to="/membership"
                          className="mt-4 w-full py-2 px-3 rounded-lg text-xs font-medium transition-all duration-200 flex items-center justify-center gap-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 hover:border-amber-500/50 no-underline"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                          </svg>
                ...
                {institutionalPartners.map((partner) => (
                  ...
                  >
                    ...
                    {isAlphaMember ? (
                      ...
                      <Link
                        to="/membership"
                        className="w-full py-2.5 px-3 rounded-lg text-xs font-semibold transition-all duration-200 flex items-center justify-center gap-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 hover:border-amber-500/50 no-underline"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                        </svg>
