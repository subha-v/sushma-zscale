...
// PDF download URL from Google Drive
const VENTURE_BENCHMARKS_PDF_URL = 'https://drive.google.com/uc?export=download&id=1de2oZAShHLh4Nuh953BeKceWL6eRyXJV';

interface VentureBenchmarksModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const VentureBenchmarksModal = ({ isOpen, onClose }: VentureBenchmarksModalProps) => {
  ...
  if (isOpen && isAlreadyCaptured) {
    ...
    link.href = VENTURE_BENCHMARKS_PDF_URL;
    link.setAttribute('download', 'zScale-Venture-Benchmarks-2026.pdf');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await fetch(GOOGLE_SCRIPT_URL, {
        ...
        body: JSON.stringify({
          formType: FORM_TYPES.VENTURE_BENCHMARKS,
          firstName,
          lastName,
          email,
          leadSource: 'Header_Venture_Benchmarks',
          leadTag: 'Venture_Benchmarks_Download',
          assetDownloaded: 'Venture_Benchmarks_PDF_2026',
          source: 'venture-benchmarks-header',
      ...
      console.error('Error submitting venture benchmarks lead:', error);
      ...
      <div>
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-white/70">
            Dallas-specific valuation multiples and venture data. Your PDF will download automatically.
          </p>
        </div>
